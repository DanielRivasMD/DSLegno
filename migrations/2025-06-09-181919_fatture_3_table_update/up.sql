...
BEGIN
  ...
  SELECT 
    ...
    SUM(CASE WHEN typo = 'VENDUTO' THEN quantita ELSE 0 END) AS somma_mc_vendita_pefc,  -- Adjust with pefc condition if needed
  ...
  -- Recalculate monthly aggregates grouped by month (taken from giorno_data).
  ...
  -- Recalculate annual aggregates grouped by year.
