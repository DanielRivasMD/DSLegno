...
fn data_tagger(tag: &Tag) -> anyResult<Tagged> {
    match (
        ...
        ("ImportoTotaleDocumento", "DatiGeneraliDocumento", "DatiGenerali") => {
            Ok(Tagged::ImportoTotale)
        }
        ...
        ("DataRiferimentoTerminiPagamento", "DettaglioPagamento", "DatiPagamento") => {
            Ok(Tagged::DataRiferimentoTermini)
        }
        ("DataScadenzaPagamento", "DettaglioPagamento", "DatiPagamento") => {
            Ok(Tagged::DataScadenzaPagamento)
        }
        ("ImportoPagamento", "DettaglioPagamento", "DatiPagamento") => Ok(Tagged::ImportoPagamento),

        // denominazione
        ("Denominazione", "Anagrafica", "DatiAnagrafici") => match tag.ggparent.as_str() {
...
fn data_extractor(data: &String, tagged: &Tagged, fattura: &mut FatturaToCapture) -> anyResult<()> {
    match tagged {
        Tagged::Descrizione => {
            ...
            if fattura.dettaglio_linee.len() == 0
                || fattura.dettaglio_linee.last().unwrap().check()?
            {
